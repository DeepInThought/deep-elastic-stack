...
### Took on purpose to fix an issue with permissions when mounting configs.  ###
### Source: https://raw.githubusercontent.com/elastic/elasticsearch-docker/master/build/elasticsearch/bin/docker-entrypoint.sh ###
set -e
...
### Updated so it will ingest a password regardless if x-pack is being setup. ###    
...
if [[ "$(id -u)" == "0" ]]; then
    ...
    ### Added else ###
        ...
        echo "ls -lat /usr/share/elasticsearch/"
    fi
fi
